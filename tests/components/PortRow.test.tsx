...
const renderRow = (port: PortEntry, isSelected: boolean) =>
  render(
    <PortRow port={port} isSelected={isSelected} colProcess={COL_PROCESS} />
  )
...
describe('PortRow', () => {
  describe('unselected row', () => {
    ...
    it('renders the user', () => {
      const { lastFrame } = renderRow(samplePort, false)
      expect(lastFrame()).toContain('patebry')
    })
  ...
  describe('process name truncation', () => {
    it('truncates a process name longer than colProcess', () => {
      const longNamePort: PortEntry = {
        ...samplePort,
        process: 'averylongprocessname_extra',
      }
  ...
  describe('user field truncation', () => {
    it('truncates user field to COL_USER (14) characters', () => {
      ...
      const longUserPort: PortEntry = {
        ...samplePort,
        user: 'averylongusername',
      }
      const { lastFrame } = render(
        <PortRow
          port={longUserPort}
          isSelected={false}
          colProcess={COL_PROCESS}
        />
      )
